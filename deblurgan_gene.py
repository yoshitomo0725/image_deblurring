...
class ResnetGenerator(nn.Module):
    def __init__(
            ...
            n_blocks=6, gpu_ids=[], use_parallel=False, learn_residual=False, padding_type='reflect'):
        assert (n_blocks >= 0)
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor) and self.use_parallel:
            output = nn.parallel.data_parallel(self.model, input, self.gpu_ids) # gpuでの並列化処理
        else:
...
class UnetGenerator(nn.Module):
    def __init__(
            ...
            use_dropout=False, gpu_ids=[], use_parallel=False, learn_residual=False):
